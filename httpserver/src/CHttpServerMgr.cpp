...
std::string CHttpServerMgr::PostOnMessage(std::string struri, std::string strmsg)
{
  ...
  else
  {
    ...
    if (jreader.parse(strmsg, jmsg))
    {
      ...
      if ("/devicemanage/get_empty_frame.php" == struri)
      {
        printf("%s %d struri:%s\n", __FUNCTION__, __LINE__, struri.c_str());
        printf("获取空车位\n");
      ...
      else if (jmsg["type"].asString() == "http_reg")
      {
        printf("%s %d struri:%s\n", __FUNCTION__, __LINE__, struri.c_str());
        printf("heartbeat\n");
        // rtvalue["force_trigger"] = 1;//强制触发
        // rtvalue["screen_shot"]["relative_url"] = "relative_url";
        // rtvalue["screen_shot"]["absolute_url"] = "/absolute_url";    //获取当前图像

        // std::string serial_data = "test http send serial";
        // int len = serial_data.length();                      //发送485数据
  ...
  // if ("/devicemanagement/php/receivedeviceinfo.php" == struri)
  // {
  //   Json::Value rtvalue;
  //   printf("heartbeat\n");
  //   if (!ivs_again)
