...
//  printf("%s %d\n", __FUNCTION__, __LINE__);
  ...
  soap->peer.in.sin_port = htons((short)port);
  soap->errmode = 0;
   printf("%s %d socket:%d peerlen:%d\n", __FUNCTION__, __LINE__, soap->master), soap->peerlen;
   int on = 1;
   setsockopt(soap->master, SOL_SOCKET, SO_REUSEADDR, &on, sizeof(on));
   int ret = bind(soap->master, &soap->peer.addr, (int)soap->peerlen);
  if (0 != ret )
  { soap->errnum = soap_socket_errno(soap->master);
    perror("bind");
   printf("%s %d Could not bind to host\n", __FUNCTION__, __LINE__);
    DBGLOG(TEST, SOAP_MESSAGE(fdebug, "Could not bind to host\n"));
    soap_closesock(soap);
    soap_set_receiver_error(soap, tcp_error(soap), "bind failed in soap_bind()", SOAP_TCP_ERROR);
    return SOAP_INVALID_SOCKET;
  ...
  printf("%s %d\n", __FUNCTION__, __LINE__);
#endif
printf("%s %d %d\n", __FUNCTION__, __LINE__, soap->omode & SOAP_IO_UDP);
  if (!(soap->omode & SOAP_IO_UDP))
  { 
    int listttenret = listen(soap->master, backlog);
    if (0!=listttenret) {
      perror("listen");
      printf("%s %d Could not bind to host\n", __FUNCTION__, __LINE__);
      soap->errnum = soap_socket_errno(soap->master);
  ...
  printf("%s %d %d  bind to host success\n", __FUNCTION__, __LINE__, soap->master);
  return soap->master;
}
#endif
#endif
  ...
  for (;;)
  {
    if (soap->accept_timeout || soap->send_timeout || soap->recv_timeout)
    {
      for (;;)
      {
        ...
        {
          printf("%s %d accept failed in soap_accept()\n", __FUNCTION__, __LINE__);
	...
	if (*s)
...
#endif
      printf("%s %d Accept socket=%d at port=%d from IP='%s'\n", __FUNCTION__, __LINE__, soap->socket, soap->port, soap->host);
      ...
      { struct linger linger;
        ...
        { soap->errnum = soap_socket_errno(soap->socket);
          printf("%s %d setsockopt SO_LINGER failed in soap_accept()\n", __FUNCTION__, __LINE__);
    ...
    {
      printf("%s %d Accept failed from %s\n", __FUNCTION__, __LINE__, soap->host);
